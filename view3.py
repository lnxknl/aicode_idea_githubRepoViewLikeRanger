    ...
    def run(self, stdscr):
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                if self.current_pane == 3:  # File content pane
                    self.scroll_offsets[3] = max(0, self.scroll_offsets[3] - self.max_lines // 2)
                    self.selected_indices[3] = max(self.scroll_offsets[3], self.selected_indices[3] - self.max_lines // 2)
            elif key == 4:  # Ctrl+D
                if self.current_pane == 3:  # File content pane
                    max_scroll = max(0, len(self.file_content) - self.max_lines + 2)
                    self.scroll_offsets[3] = min(max_scroll, self.scroll_offsets[3] + self.max_lines // 2)
                    self.selected_indices[3] = min(len(self.file_content) - 1, self.selected_indices[3] + self.max_lines // 2)
...
print("GitHub repository viewer (Ranger-like) script with detailed file changes and Ctrl+U/Ctrl+D navigation created successfully. You can now run it with a GitHub username as an argument.")
